...
// Evaluation
void SJF_evaluate(Process* p, int n) {
...
void SJF_Scheduling(Process* p, int n) {
    ...
    int completed_count = 0;  // 완료된 프로세스들 수
    int current_time = 0;

    while (completed_count < n) {
        int idx = -1;
        int min_burst = INT_MAX;  // min_burst 초기값

        for (int i = 0; i < n; i++) {
            if (p[i].arrival_time <= current_time && !p[i].completed) {  // 도착했고 완료 아닌 프로세스들만 고려, 그 중 min_burst 프로세스 택
                if (p[i].burst_time < min_burst) {
                    min_burst = p[i].burst_time;
                    idx = i;
                }
                else if (p[i].burst_time == min_burst && p[i].arrival_time < p[idx].arrival_time) {  // burst_time 같으면 먼저 도착한 프로세스 택
                    idx = i;
                }
            }
        }
